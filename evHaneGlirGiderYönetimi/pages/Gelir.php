        ...
        <body>
            <div class="container-fluid">
                <div class="col-lg-6 col-md-6 col-sm-6 col-xs-6">
                    <h3 style="margin-top: 10px;">Kişisel Gelir Gider Yönetimi</h3>
                </div>
                <div class="col-lg-6 col-md-6 col-sm-6 col-xs-6">
                ...
                <div class="clear"></div>            
                <h3>GELİR Kayıtlar</h3>
                <table id="gelirExcel" class="table table-striped">
                    <tbody>
                                    <tr>
                                        <th>Ay / Yıl</th>
                                        <th>Tarih</th>
                        ...
                        <?php 
                        $kullaniciId = $_SESSION['kullaniciId'];
                        $query = $conn->query("SELECT * FROM kullanicigelirleri WHERE kullanicigelirId = $kullaniciId");
                        $sorusayisi = 1;
                        $toplamGelir = 0;
                        $tr = "</tr><tr>";
                        if($query->rowCount() > 0){
                            while($row = $query->fetch()){
                                $gelirMiktar = $row['gelirMiktar'];
                                $gelirkategoriAd = $row['gelirkategoriAd'];
                                $gelirDate = $row['gelirDate'];
                                $gelirId  = $row['gelirId'];
                                $gelirAciklama = $row['gelirAciklama'];
                                $toplamGelir += $gelirMiktar;
                                ?>
                                <tr>
                                    <td><?php echo strftime('%B/%Y',time())?></td>
                                    <td><?php echo $gelirDate?></td>
                                    <td><?php echo $gelirkategoriAd ?></td>
                                    <td><?php echo $gelirAciklama ?></td>
                                    <td style="text-align: center;">
                                        <a href="deleteGelir.php?id=<?php echo $gelirId ?>" class="btn btn-danger" title="Sil" >SİL</a>
                                    </td>
                                    <td style="text-align: right;"><?php echo $gelirMiktar ?></td>
                                </tr>
                                <?php
                                }
                            }else{ 
                            ...
                            <?php } ?> 
                                ...
                                <th></th>
                                <th></th>
                                <th style="text-align: right;" colspan="3">TOPLAM: <?php echo $toplamGelir?></th>
                            </tr>
                    </tbody>
                </table>
                </div>
